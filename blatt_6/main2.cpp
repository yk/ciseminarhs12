...
Mat calculateFundamental() {
	...
	fundamental8 = findFundamentalMat(matchedPoints_1, matchedPoints_2,
			CV_FM_8POINT);
	...
	for (int i = 0; i < matchedPoints_1.size(); i++) {
		//should all be very close to 0!
		checkFundamentalMatrix(matchedPoints_1[i], matchedPoints_2[i],
				fundamentalRan);
...
Mat estimateFund(int numCor = 0) {
	Mat avg(4, 1, CV_32F);
	vector<Mat> cors;
	random_shuffle(matchedPoints_1.begin(), matchedPoints_1.end());
	int n;
	if (numCor == 0) {
		n = matchedPoints_1.size();
	} else {
		n = numCor;
	}
	for (int i = 0; i < n; i++) {
		Mat m(4, 1, CV_32F);
		Point2f mp1 = matchedPoints_1[i];
		Point2f mp2 = matchedPoints_2[i];
		m.at<float>(0, 0) = mp2.x;
		m.at<float>(1, 0) = mp2.y;
		m.at<float>(2, 0) = mp1.x;
		m.at<float>(3, 0) = mp1.y;
		avg += m;
		cors.push_back(m);
	}
	avg /= n;
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < 4; j++) {
			A.at<float>(i, j) = cors[i].at<float>(j, 0);
		}
	}
	Mat u, s, vt;
	SVD::compute(A, s, u, vt);
	Mat Nt(0, vt.cols, CV_32F);
	Nt.push_back(vt.row(vt.rows - 1));
	Mat e = -1.0 * Nt * avg;
	Mat fund = Mat::zeros(3, 3, CV_32F);
	fund.at<float>(0, 2) = Nt.at<float>(0, 0);
	fund.at<float>(1, 2) = Nt.at<float>(0, 1);
	fund.at<float>(2, 0) = Nt.at<float>(0, 2);
	fund.at<float>(2, 1) = Nt.at<float>(0, 3);
	fund.at<float>(2, 2) = e.at<float>(0, 0);
	return fund;
}

double getFundError(Mat& f) {
	double error = 0;
	for (int i = 0; i < matchedPoints_1.size(); i++) {
		auto mp1 = matchedPoints_1[i];
		auto mp2 = matchedPoints_2[i];
		Mat x(3, 1, CV_32F);
		x.at<float>(0, 0) = mp1.x;
		x.at<float>(1, 0) = mp1.y;
		x.at<float>(2, 0) = 1.0;
		Mat xpt(1, 3, CV_32F);
		xpt.at<float>(0, 0) = mp2.x;
		xpt.at<float>(0, 1) = mp2.y;
		xpt.at<float>(0, 2) = 1.0;
		Mat r = xpt * f * x;
		error += r.at<float>(0, 0);
	}
	return error;
}
...
void advanced3() {
	...
	for (int i = 0; i < numTries; i++) {
		auto f = estimateFund(5);
		double score = getFundError(f);
		if (score < bestScore) {
			bestF = f;
			bestScore = score;
		}
